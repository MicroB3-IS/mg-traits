...
source /bioinf/projects/megx/mg-traits/resources/config_files/config_finish.bash
source ./01-subjobs_env
source /bioinf/projects/megx/mg-traits/resources/config_files/config.functions.bash

# if [[ ! -e ./01-environment ]]; then
# 	echo "01-environment is missing from work dir: ${WORK_DIR}"
# 	cleanup && exit 2
# 	email_comm "no 01-environment is missing from work dir: ${WORK_DIR}"
# 	
# else
# 	cat 01-environment
...
SLV_CLASSIFY_RESULTS=$(ls -1 06-part-1.fasta | wc -l)
...
# RS found in the metagenome. Job ${JOB_ID}"
...
if [[ "$?" -ne "0" ]]; then
  email_comm "Error inserting ${JOB_ID}"
...
t\$sample_label <- NULL
row.names(t)<- t\$id
t\$id <- cbind(rownames(pca.scores), "codon-usage", pca.scores, "${MG_ID}")
