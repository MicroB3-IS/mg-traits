...
show_usage(){
  cat <<EOF
  Usage: ${0##*/} [-h] [-c|--config FILE] [-f --functions FILE] \
[-i|--inputdir DIR] [-o|--outdir DIR] [-l|--log FILE] ...

  -h|--help          display this help and exit
  -c|--config        configuration file
  -f|--functions     functions file 
  -i|--inputdir      input fasta file
  -o|--outdir        output fasta file
  -l|--log           log file
EOF
}
...
read_config(){
  ...
  if [[ ! -r "${CONFIG_FILE}" ]] || [[ ! -r "${FUN_FILE}" ]]; then
    echo "${CONFIG_FILE} or ${FUN_FILE} doesn't exist or is not readable"
    exit 1
  fi
...
FAA_RESULTS=$( find "${MG_TRAITS_DIR}" -name "05-part*.faa" | wc -l )
FFN_RESULTS=$( find "${MG_TRAITS_DIR}" -name "05-part*.ffn" | wc -l )
SLV_CLASSIFY_RESULTS=$( find "${MG_TRAITS_DIR}" -name "06-part-1.fasta" | wc -l)
SUBJOBS=$( find "${MG_TRAITS_DIR}" -name "05-part*.fasta" | wc -l)
...
if [[ "${FAA_RESULTS}" -ne "${SUBJOBS}" ]] || \
  ...
  db_error_comm "At least one of the subjobs did not yield results."
  error_exit "At least one of the subjobs did not yield results." 1; exit
fi
...
if [[ "${NUM_GENES}" -eq "0" ]]; then
  db_error_comm "No genes predicted in the sample. NUM_GENES ${NUM_GENES} = 0"
  error_exit "No genes predicted in the sample. NUM_GENES ${NUM_GENES} = 0" 1;
  exit
fi
...
NUM_RNA=$( cat 06-part-*.classify.fasta | grep -c '>' )
printf "Number of genes: %d\n" "${NUM_RNA}"

if [[ "${NUM_RNA}" -eq "0" ]]; then
  db_error_comm "No RNA found in the sample. NUM_RNA ${NUM_RNA} = 0"
  error_exit "No RNA found in the sample. NUM_RNA ${NUM_RNA} = 0" 1; exit
fi
...
if [[ "$?" -ne "0" ]]; then
  db_error_comm "UProCf error. uproc -t ${NSLOTS} -p -l -O 2 -P 3 -o \
${PFAMFILERAW} ${uproc_pfam} ${uproc_model} ${GENENT} failed. Job ${JOB_ID}."
  error_exit "UProCf error. uproc -t ${NSLOTS} -p -l -O 2 -P 3 -o \
${PFAMFILERAW} ${uproc_pfam} ${uproc_model} ${GENENT} failed. Job ${JOB_ID}." \
  1; exit
fi
...
if [[ "$?" -ne "0" ]]; then
  email_comm "cusp --auto -stdout ${GENENT} |awk '{if ($0 !~ * && $0 !~ /[:alphanum:]/ && $0 !~ /^$/){print $1,$2,$5}}'\ 
> ${CODONCUSP} failed. Job ${JOB_ID}}"
...
###########################################################################################################
compseq --auto -stdout -word 1 "${RAW_FASTA}" | awk '{if (NF == 5 && $0 ~ /^A|T|C|G/ && $0 !~ /[:alphanum:]/ ) {print $1,$2,$3}}' \
 > "${NUC_FREQS}"

if [[ "$?" -ne "0" ]]; then
  email_comm "compseq --auto -stdout -word 1 ${RAW_FASTA} |awk '{if (NF == 5 && $0 ~ /^A|T|C|G/ && $0 !~ /[:alphanum:]/){print $1,$2,$3}}'\
  > ${NUC_FREQS} failed. Job ${JOB_ID}"
...
compseq --auto -stdout -word 2 "${RAW_FASTA}" |awk '{if (NF == 5 && $0 ~ /^A|T|C|G/ && $0 !~ /[:alphanum:]/ ) {print $1,$2,$3}}' \
> "${DINUC_FREQS}"

if [[ "$?" -ne "0" ]]; then 
  email_comm "compseq --auto -stdout -word 2 ${RAW_FASTA} |awk '{if (NF == 5 && $0 ~ /^A|T|C|G/ && $0 !~ /[:alphanum:]/ ) {print $1,$2,$3}}' \
  > ${DINUC_FREQS} failed. Job ${JOB_ID}"
...
cat 06-part-*.classify.fasta | grep lca_tax_slv | cut -d '=' -f 2 | sort | uniq -c | awk '{print substr($0, index($0, $2))"=>"$1}' \
...
cat 06-part-*.classify.fasta | grep lca_tax_slv | cut -d '=' -f 2 | awk 'BEGIN{FS=";"}{if (NF > 4) {print $4}}' | sort | uniq -c \
...
tail -n1 "${CODON_TABLE}" | awk -vI="${MG_ID}" -vO="${SAMPLE_LABEL}" '{print I"\t"O"\t"$0}' | psql -U "${target_db_user}"\
 -h "${target_db_host}" -p "${target_db_port}" -d "${target_db_name}" -c "\COPY mg_traits.mg_traits_codon FROM STDIN"
...
tail -n1 "${AA_TABLE}" | awk -vI="${MG_ID}" -vO="${SAMPLE_LABEL}" '{print O"\t"$0"\t"I}' | psql -U "${target_db_user}" \
-h "${target_db_host}" -p "${target_db_port}" -d "${target_db_name}" -c "\COPY mg_traits.mg_traits_aa FROM STDIN"

tail -n1 "${AA_TABLE}" | awk -vI="${MG_ID}" -vO="${SAMPLE_LABEL}" \
'{print O"\t"$0"\t"I}' | psql -U "${target_db_user}" \
-h "${target_db_host}" -p "${target_db_port}" -d "${target_db_name}" -c \
"\COPY epereira.mg_traits_aa FROM STDIN"
...
printf "%s\t%s\t" "${MG_ID}" "${SAMPLE_LABEL}" | cat - "${PFAMDB}" | psql -U "${target_db_user}" -h "${target_db_host}" -p \
"${target_db_port}" -d "${target_db_name}" -c "\COPY mg_traits.mg_traits_functional FROM STDIN CSV delimiter E'\t'"
...
paste "${SLV_TAX_ORDER}" "${SLV_TAX_RAW}" | awk -vI="${MG_ID}" -vO="${SAMPLE_LABEL}" '{print I"\t"O"\t"$0}' | psql -U \
"${target_db_user}" -h "${target_db_host}" -p "${target_db_port}" -d "${target_db_name}" \
-c "\COPY mg_traits.mg_traits_taxonomy FROM STDIN CSV delimiter E'\t'"
...
printf "%s\t" "${SAMPLE_LABEL}" | cat - "${ODDS_TABLE}" | awk -vI="${MG_ID}" '{print $0"\t"I}' | psql -U "${target_db_user}"\
 -h "${target_db_host}" -p "${target_db_port}" -d "${target_db_name}" -c "\COPY mg_traits.mg_traits_dinuc FROM STDIN"
...
${ABRATIO}, ${PERCTF}, ${PERCCL}, ${MG_ID});" | psql \
...
###########################################################################################################
# 20 - PCAs. We calculate the PCAs if we have more than 30 metagenomes in the database
# For functional and taxonomy we apply hellinger transformation to the data before PCA
###########################################################################################################

# Get existing data for CODON

psql -U "${target_db_user}" -h "${target_db_host}" -p "${target_db_port}" -d "${target_db_name}" -c "\COPY (select C.* \
from mg_traits.mg_traits_codon C inner join mg_traits.mg_traits_jobs_public P on C.id = P.id) TO ${PCA_CODON_FILE} CSV \
HEADER delimiter E'\t'"
...
# For AA
  psql -U "${target_db_user}" -h "${target_db_host}" -p "${target_db_port}" -d "${target_db_name}" -c "\COPY (select \
  A.* from mg_traits.mg_traits_aa A inner join mg_traits.mg_traits_jobs_public P on A.id = P.id) TO ${PCA_AA_FILE} CSV \
  HEADER delimiter E'\t'"
...
# For DINUC
  psql -U "${target_db_user}" -h "${target_db_host}" -p "${target_db_port}" -d "${target_db_name}" -c "\COPY (select \
  D.* from mg_traits.mg_traits_aa D inner join mg_traits.mg_traits_jobs_public P on D.id = P.id) TO ${PCA_DINUC_FILE} CSV\
  HEADER delimiter E'\t'"
...
# For FUNCTIONAL
  psql -U "${target_db_user}" -h "${target_db_host}" -p "${target_db_port}" -d "${target_db_name}" -c "\COPY (SELECT \
  F.id,(each(functional)).key as key, (each(functional)).value FROM mg_traits.mg_traits_functional F inner join \
  mg_traits.mg_traits_jobs_public P on F.id = P.id order by id, key) TO ${PCA_FUNCTIONAL_FILE} CSV HEADER delimiter E'\t'"
...
# For TAXONOMY
  psql -U "${target_db_user}" -h "${target_db_host}" -p "${target_db_port}" -d "${target_db_name}" -c "\COPY (SELECT \
  T.id,(each(taxonomy_order)).key as key, (each(taxonomy_order)).value FROM mg_traits.mg_traits_taxonomy T inner join \
  mg_traits.mg_traits_jobs_public P on T.id = P.id order by id, key) TO $PCA_TAXONOMY_FILE CSV HEADER delimiter E'\t'"
...
echo "UPDATE mg_traits.mg_traits_jobs SET time_finished = now(), return_code = 0, total_run_time = total_run_time + \
${RUN_TIME}, time_protocol = time_protocol || ('${JOB_ID}', 'mg_traits_finish', ${RUN_TIME})::mg_traits.time_log_entry\
 WHERE sample_label = '${SAMPLE_LABEL}' AND id = '${MG_ID}';" | psql -U "${target_db_user}" -h "${target_db_host}" -p \
"${target_db_port}" -d "${target_db_name}"

if [[ "$?" -ne "0" ]]; then
    email_comm "Error inserting successful_jobs_key. Job ${JOB_ID}"
    db_error_comm "Error inserting successful_jobs_key"
    cleanup && exit 1;
fi
...
if [[ "$?" -ne "0" ]]; then
    email_comm "Error moving files mv ${THIS_JOB_TMP_DIR} ${FINISHED_JOBS_DIR}"
    db_error_comm "Error moving files mv ${THIS_JOB_TMP_DIR} ${FINISHED_JOBS_DIR}"
    cleanup && exit 1;
fi


TOTAL_TIME=$( psql -t -U "${target_db_user}" -h "${target_db_host}" -p "${target_db_port}" -d "${target_db_name}" -c \
"select (time_finished - time_started) from mg_traits.mg_traits_jobs WHERE sample_label = '${SAMPLE_LABEL}' AND id = '${MG_ID}';" | tr -d ' ')
