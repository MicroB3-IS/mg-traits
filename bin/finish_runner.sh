...
###########################################################################################################
# 0 - Change to working directory: -cwd gives Eqw  may be due to excessive write to network mounted storage?
###########################################################################################################

THIS_JOB_TMP_DIR="$1"
cd "${THIS_JOB_TMP_DIR}"
if [[ "$?" -ne "0" ]]; then 
  email_comm "Can't chdir ${THIS_JOB_TMP_DIR}"
  db_error_comm "Can't chdir ${THIS_JOB_TMP_DIR}"
  cleanup && exit 1;
fi
...
if [[ -r "${CONFIG}" && -r "${CONFIG_FINISH}" && -r "${SUBJOBS_ENV}" ]]; then
  source "${CONFIG}"
  source "${CONFIG_FINISH}"
  source "${SUBJOBS_ENV}"
else
  mail -s "mg_traits:${JOB_ID} failed" epereira@mpi-bremen.de <<EOF
"Missing configuration files: ${CONFIG}; ${CONFIG_FINISH}; ${SUBJOBS_ENV}"
EOF
  exit 1
fi
...
FAA_RESULTS=$( find . -name "05-part*.faa" | wc -l )
FFN_RESULTS=$( find . -name "05-part*.ffn" | wc -l )
SLV_CLASSIFY_RESULTS=$( find . -name "06-part-1.fasta" | wc -l)
SUBJOBS=$( find . -name "05-part*.fasta" | wc -l)
...
if [[ "${FAA_RESULTS}" -ne "${SUBJOBS}" ]] || [[ "${FFN_RESULTS}" -ne "${SUBJOBS}" ]]; then  # || [[ "${SLV_CLASSIFY_RESULTS}" -ne "${SUBJOBS}" ]]; then
...
module load uproc/1.2 
##### load uproc module #####

uproc-dna -t "${NSLOTS}" -p -l -O 2 -P 3 -o "${PFAMFILERAW}" "${uproc_pfam}" "${uproc_model}" "${GENENT}" 
# "${uproc}" -t "${NSLOTS}" -p -l -O 2 -P 3 -o "${PFAMFILERAW}" "${uproc_pfam}" "${uproc_model}" "${GENENT}" 

if [[ "$?" -ne "0" ]]; then
  email_comm "UProCf error. uproc -t ${NSLOTS} -p -l -O 2 -P 3 -o ${PFAMFILERAW} ${uproc_pfam} ${uproc_model} \
${GENENT} failed. Job ${JOB_ID}."
  db_error_comm "UProC error. Please contact adminitrator"
  cleanup && exit 1;
fi

echo "Extracting PFAMS... ${PFAMFILE}"
...
echo "INSERT INTO mg_traits.mg_traits_results (sample_label, gc_content, gc_variance, num_genes, total_mb, num_reads, \
ab_ratio, perc_tf, perc_classified, id) VALUES ('${SAMPLE_LABEL}',${GC},${VARGC}, ${NUM_GENES}, ${NUM_BASES}, ${NUM_READS}, ${ABRATIO}, ${PERCTF}, ${PERCCL}, ${MG_ID});"\
