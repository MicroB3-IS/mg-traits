...
if [[ -s "pear.unassembled.forward.fastq" ]]; then
  ...
  PE2_qc_nonmerged="R2_qc_nonmerged.fasta"
    ...
    bbduk.sh  in="pear.unassembled.forward.fastq" in2="pear.unassembled.reverse.fastq" \
...
# || ('${JOB_ID}', 'mg_traits', ${RUN_TIME})::mg_traits.time_log_entry WHERE sample_label = '${SAMPLE_LABEL}' AND job_id = '${JOB_ID}';" \
# | psql -U "${target_db_user}" -h "${target_db_host}" -p "${target_db_port}" -d "${target_db_name}"
