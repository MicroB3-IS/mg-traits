...
if [[ \$? -ne "0" ]]; then 
  email_comm "SINA alignment failed. Please contact administrator"
...
if [[ "\${NUM_RNA_ALIGN}" -eq 0 ]]; then 
  email_comm "No aligned RNA sequences by sina"
...
if [[ \$? -ne "0" ]]; then 
  email_comm "SINA classify failed. Please contact administrator"
...
if [[ "\${NUM_RNA_CLASSIFY}" -eq 0 ]]; then 
  email_comm "No classified RNA sequences by sina"
